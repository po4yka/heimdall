...
enum WidgetContentState {
    case snapshot(WidgetSnapshot)
    case empty
    case failure(String)
}

struct SingleProviderWidgetEntry: TimelineEntry {
    let date: Date
...
struct SingleProviderTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> SingleProviderWidgetEntry {
        SingleProviderWidgetEntry(date: Date(), provider: .claude, state: .empty)
    }

    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> SingleProviderWidgetEntry {
    ...
    func timeline(for configuration: ProviderSelectionIntent, in context: Context) async -> Timeline<SingleProviderWidgetEntry> {
