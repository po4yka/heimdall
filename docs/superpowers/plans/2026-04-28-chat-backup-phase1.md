...
**Goal:** Ship a content-addressed daily snapshot of every CLI provider's on-disk session files into `~/.heimdall/archive/`, surfaced via CLI subcommands, the SwiftBar menubar, and a new dashboard "Backup" tab, with optional launchd/cron integration through the existing scheduler.

**Architecture:** A new `src/archive/` module owns the object store, manifests, and snapshot/restore/prune/verify pipelines. Each `Provider` gains an `archive_paths()` method so the snapshotter can discover source files generically. Scheduler grows a new `ARCHIVE_JOB` constant alongside `SCAN_JOB`. The web server gains three loopback-only endpoints (`GET/POST /api/archive*`). The dashboard adds a fifth tab with a single panel. The menubar gets a snapshot section.

**Tech Stack:** Rust 2024, `clap`, `serde`/`serde_json`, `sha2`, `walkdir`, `tempfile`, `axum`, `tracing`, `rusqlite`, Preact + Tailwind v4 + esbuild.

Spec reference: `docs/superpowers/specs/2026-04-28-chat-backup-design.md` sections 4–5, 8, 9, and Phase 1 of section 10.
...
**Test files (one per source file):** standard Rust convention `#[cfg(test)] mod tests` blocks at the bottom of each new `src/archive/*.rs` file.

---

## Conventions used by this plan

- All Rust changes follow the project's existing rules (CLAUDE.md): `thiserror` for new error types, `anyhow` in CLI dispatch, no `.unwrap()` in library code, `tempfile` in tests, `tracing::{debug,info,warn}` for logs, Rust edition 2024.
- All SQL in `src/scanner/db.rs` is the existing rule. `index.sqlite` is a *separate* SQLite file owned by `src/archive/index.rs`, so the rule does not apply — but every SQL string in the archive module lives in `index.rs` and nowhere else, mirroring the spirit of the existing rule.
- TDD: every task starts with a failing test. Run it red. Then implement the minimum that makes it green. Run it green. Commit.
- Conventional-commit prefixes match the existing history: `feat(archive):`, `feat(scheduler):`, `feat(server):`, `feat(ui):`, `feat(menubar):`, `test(archive):`, `chore(deps):`.
...
(Field name matches whatever the existing struct field is — read the file first; if the field is private under a different name, expose it via the same accessor used by `discover_sessions`.)

- [ ] **Step 6: Repeat for the remaining providers**

For each of `codex.rs`, `cursor.rs`, `opencode.rs`, `pi.rs`, `copilot.rs`, `xcode.rs`, `amp.rs`:
...
For SQLite-backed providers (Cursor, OpenCode), `archive_paths()` returns the *parent directory* containing the `state.vscdb` / database file — the snapshotter then captures the DB file plus any sidecar journals (`-wal`, `-shm`) as a unit.
...
`src/main.rs` re-declares lib modules so the binary crate sees them too (see `src/lib.rs` header comment). Find the cluster of `mod` lines near the top of `main.rs` (`mod scanner;`, `mod scheduler;`, etc.) and add:
...
Also create `src/archive/index.rs` as a stub now so the module tree declared in `mod.rs` resolves:
...
(If the module didn't compile, fix the bracket/import. The test should pass on first build because the implementation is included with the test. The point of TDD here is that the next step adds *new* behavior with its own test.)
...
In `src/main.rs`, find the `enum Commands` block (line ~40). Add a new variant **after** the existing `Optimize` variant:
...
In `src/main.rs`, locate the giant `match cli.command { ... }` in `fn main()`. Add a new arm (placement matches the order of the enum variants):
...
Locate the `Scheduler` subcommand area in `src/main.rs`. Find the `SchedulerAction` enum (search for `enum SchedulerAction`). Add the flag to the `Install` variant:
...
Then in the `SchedulerAction::Install { ... } => { ... }` dispatch arm, add a second `install()` call when the flag is set:
...
Mirror the change for the `SchedulerAction::Uninstall` arm — also call `current_for(ARCHIVE_JOB).uninstall()?` so a re-uninstall removes both.
...
Append to `src/server/api.rs` (near the other write-style handlers like `api_rescan` and `api_live_provider_refresh`):
...
In `src/server/mod.rs::build_router`, add three routes alongside the existing ones:
...
(If `mk_test_state` does not exist with that name, follow the existing test-state factory in `server/tests.rs` — it will already be in use by neighboring tests.)
...
In `src/ui/state/store.ts`, find the `DashboardTab` union (search for `type DashboardTab`). Replace it with:

```ts
export type DashboardTab = 'overview' | 'activity' | 'breakdowns' | 'tables' | 'backup';
```

In the same file, add a backup signal alongside the existing dashboard state signals (look for the block of `export const ... = signal(...)`):
...
In `src/ui/dashboard/runtime.ts`, locate `handleDashboardTabChange` and add a `'backup'` branch that mounts the panel:
...
(Match the exact way other tabs render — read `runtime.ts` first to find the actual mount point and the existing `tab ===` branch shape.)
...
Add these two fields to the `MenubarData { ... }` literal at the bottom of `run_menubar()`.

- [ ] **Step 4: Update existing tests for the new fields**

Existing tests construct `MenubarData { ... }` without these fields. Because the struct derives `Default`, callers using `..Default::default()` are fine, but explicit literals will fail to compile. Fix each broken test by adding:
...
The new submenu rows must fit `MENUBAR_ROW_MAX_CHARS=80`. The "Last: 2026-04-28T08:00:00Z (12345678 bytes)" line is ~50 chars — well under budget. But verify with:
...
All must pass. Fix any warnings inline; if clippy flags an issue in existing code that's unrelated to this phase, leave a comment in your fixup commit explaining why it's drive-by-fixed.
...
Visit `http://localhost:8087/`, click the **Backup** tab, confirm the snapshot appears, click **Snapshot now**, confirm a new row.
...
You should see ~12 commits forming a clean Phase 1 history. Phase 2 (account-export importer) is its own plan and gets its own session.
...
- The `index.sqlite` cross-tier index is described in §4 but Phase 1 doesn't need it — the dashboard reads manifests directly via `/api/archive`. Promoting the index from "rebuildable" to "required" belongs to Phase 2 when the importer needs to query across tiers. The plan ships an empty stub at `src/archive/index.rs` so the module layout matches the spec and Phase 2 has a place to land code.

**Placeholder scan:** Searched for "TODO", "TBD", "implement later", "add appropriate". One TODO note remains in the self-review (the `index.sqlite` deferral) — that's a deliberate cross-phase acknowledgement, not a step gap.

**Type consistency:** `Sha256Hash` (objects.rs), `Manifest`/`Summary` (manifest.rs), `Archive`/`SnapshotMeta` (mod.rs), `BackupPanelProps` (BackupPanel.tsx), `SnapshotMeta` (store.ts) — names match across tasks. `DashboardTab` union extension is consistent between Tasks 10's store.ts edit and the existing `DashboardTabs.tsx` registration.

**Module placement:** the empty `src/archive/index.rs` referenced in the File Structure table is created as part of Task 2 step 2 (the module skeleton declares it), but the file itself is left as a one-liner docstring stub:
