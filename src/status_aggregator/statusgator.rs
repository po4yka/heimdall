...
pub(crate) fn fetch_live(config: &AggregatorConfig, api_key: &str) -> CommunitySignal {
    ...
    rt.block_on(async move {
        ...
        for slug in &claude_slugs {
            claude_signals.push(fetch_slug_async(&client, slug, &api_key_owned).await);
        }

        let mut openai_signals = Vec::with_capacity(openai_slugs.len());
        for slug in &openai_slugs {
            openai_signals.push(fetch_slug_async(&client, slug, &api_key_owned).await);
        }
...
/// Fetch a single service slug from the StatusGator API (async).
async fn fetch_slug_async(client: &reqwest::Client, slug: &str, api_key: &str) -> ServiceSignal {
    let url = format!("{}/{}/status", STATUSGATOR_BASE, slug);
